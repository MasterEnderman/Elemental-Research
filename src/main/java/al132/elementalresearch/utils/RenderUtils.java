package al132.elementalresearch.utils;
...
public class RenderUtils {
    ...
    /*
        public static int renderText(Minecraft mc, int x, int y, String txt, Color color) {
            ...
            mc.fontRenderer.drawString(txt, x, y, color.getRGB());
            GlStateManager.enableLighting();
    ...
    */
    ...
    public static boolean renderItemStack(Minecraft mc, RenderItem itemRender, ItemStack itm, int x, int y, String txt, boolean highlight) {
        ...
        if (highlight) {
            GlStateManager.disableLighting();
            drawVerticalGradientRect(x, y, x + 16, y + 16,0x30ddddff,0x80ffffff); //Al132 <- |old ->0x80ffffff, 0xffffffff);
        }
...
//        this.zLevel = 300.0F;
    ...
    public static int renderText(Minecraft mc, int x, int y, String txt) {
        return renderText(mc, x, y, txt, 16777215);
    }

    public static int renderText(Minecraft mc, int x, int y, String txt, int textColor) {
        GlStateManager.color(1.0F, 1.0F, 1.0F);
        ...
        GlStateManager.disableBlend();
        int width = mc.fontRenderer.getStringWidth(txt);
        mc.fontRenderer.drawStringWithShadow(txt, x, y, textColor);
        GlStateManager.enableLighting();
        GlStateManager.enableDepth();
        // Fixes opaque cooldown overlay a bit lower
        // TODO: check if enabled blending still screws things up down the line.
